...
- `skills/`: GitHub Pages root. Each skill = `<name>/SKILL.md` + `<name>/references/*.md`
  - `wind-ui/`: Wind UI className system (skill v2.11.0, `fluttersdk_wind` 1.3.0)
  - `magic-framework/`: Magic Framework IoC + Facades + ORM (skill v0.1.4, `magic` 0.0.6)
  - `fluttersdk-dusk/`: E2E Flutter app driver via MCP (skill v0.0.9, `fluttersdk_dusk` 0.0.9)
  - `fluttersdk-telescope/`: Runtime inspector, 9 ring buffers (skill v0.0.4, `fluttersdk_telescope` 0.0.4)
  - `fluttersdk-artisan/`: CLI framework + stdio MCP server (skill v0.0.3, `fluttersdk_artisan` 0.0.9)

> Every `skills/<name>/` directory is a MIRROR of the upstream package's own `skills/<name>/` (see `.github/workflows/sync.yml`). Edit the skill in the upstream repo (`fluttersdk/wind`, `magic`, `dusk`, `telescope`, `artisan`), never here: `sync.yml` rsyncs with `--delete`, so a registry-only file is one auto-sync away from deletion.
